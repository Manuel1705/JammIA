...
Al termine del caricamento (`create_fulltext_indexes`) vengono creati due *indici full-text* Lucene, `operaNameIndex` su `Opera.name` e `museoNameIndex` su `Museo.name`, con `CREATE FULLTEXT INDEX ... IF NOT EXISTS` (idempotente: `DETACH DELETE` non rimuove gli indici). Questi indici abilitano la ricerca *fuzzy* dei titoli in fase di interrogazione (v. modulo RAG).
...
- *Marcatura dell'ambito (`in_scope`) con verifica prima di scartare.* Ogni sotto-domanda è etichettata come dentro o fuori dominio. È marcata fuori ambito (`in_scope: false`) solo quando è generica su un altro artista o tema *senza nominare alcuna opera o museo specifico* ("...e quante ne ha fatte Botticelli?"): queste parti non toccano il grafo e ricevono una nota esplicita. Se invece la sotto-domanda nomina un'opera o un museo preciso, resta in ambito *anche se attribuita a un artista non trattato*, perché quel titolo va verificato nel grafo: potrebbe essere in realtà un'opera di Caravaggio o Caracciolo attribuita per errore. In tal caso la sotto-domanda è riscritta per cercare l'opera per nome (senza vincolare l'artista sbagliato) e per chiedere il vero autore, riportando fra parentesi l'attribuzione dell'utente così che la sintesi possa correggerla.
...
- *Ricerca fuzzy di opere e musei.* I titoli non sono cercati né per uguaglianza né con `CONTAINS` sulla frase intera, ma tramite gli indici *full-text* Lucene con operatore fuzzy `~`. La stringa di ricerca è costruita scartando articoli e preposizioni ("di"/"della"/"il"...) e rendendo ogni parola di CONTENUTO sia *obbligatoria* (prefisso `+`) sia tollerante ai refusi (suffisso `~`): `CALL db.index.fulltext.queryNodes("operaNameIndex", "+sette~ +opere~ +misericordia~") YIELD node, score ... ORDER BY score DESC LIMIT 1`. Questo risolve due esigenze opposte in un colpo solo: una preposizione diversa ("della" invece di "di") o un errore di battitura (`Flagellazzione`) combaciano comunque grazie alla distanza di Levenshtein, mentre un titolo con una parola-soggetto inventata (es. `Flagellazione di Babbo Natale`) rende obbligatoria una parola inesistente e restituisce quindi *zero righe*, ricadendo pulitamente nel caso "opera non trovata" invece di essere spacciato per un'opera reale. Il vero autore (`a.name`) è sempre restituito, per verificare e correggere eventuali attribuzioni errate.
...
- *titolo esatto e correzioni*: l'opera va sempre nominata col titolo esatto restituito dai dati (`o.name`), mai con la forma scritta dall'utente; se quest'ultima differiva (preposizione diversa, refuso), la risposta lo segnala con garbo indicando la forma corretta. Analogamente, se i dati mostrano un autore diverso da quello che l'utente attribuiva, la risposta corregge l'attribuzione;
- *verifica del soggetto (rete di sicurezza)*: i titoli inventati sono già scartati a monte dalla query (parole di contenuto obbligatorie, v. modulo RAG), ma come ulteriore difesa la sintesi confronta comunque la parola-soggetto chiesta con il titolo nei dati: se differiscono (es. "Babbo Natale" vs "Cristo") non spaccia il risultato per l'opera richiesta e dichiara che quella richiesta non esiste, eventualmente citando quella simile realmente presente;
